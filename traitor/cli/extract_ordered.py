...
def run_single_ordered(
    ...
    with warnings.catch_warnings(record=True) as w:
        ...
        sorted_contours = sort_contours(contours) # Sort contours by position
        ...
        if contour_output:
            ...
            contours_cv2 = [c[:, [1, 0]].astype(np.int32) for c in sorted_contours]
...
def single_ordered_wrapped(args):
    run_single_ordered(*args)


@command(
    "extract_ordered",
    help="Detect objects and extract their contours/masks from images. Follows a bottom-to-top, left-to-right ordering for extracted contours.",
...
@option_group(
    ...
    option(
        ...
        type=int,
        default=50,
        help="Tolerance in pixels for grouping contours into rows based on y-centers (for ordering).",
        show_default=True,
    ),
)
...
def extract_ordered(
    ...
    with multiprocessing.Pool(processes=n_proc) as pool:
        list(
            tqdm(
                pool.imap_unordered(single_ordered_wrapped, args_list),
